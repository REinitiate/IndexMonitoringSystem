...
public class StockProfileService extends SqlSessionDaoSupport {
	 ...
	 * @return
	 */
	public JSONObject GetRiskProfileJson(String u_cd, String u_cd_bm, String t0, String t1){
		
		JSONObject jsonResult = new JSONObject();		
		HashMap<String, Object> timeSeriesData = GetIdxMonthlyTimeSeriesWithBm(u_cd, u_cd_bm, t0, t1);
	 ...
	 * @return
	 */
	public JSONObject GetReturnProfileJson(String u_cd, String u_cd_bm, String t0, String t1){
		...
		String[] arrTemp = GetMinMaxDt(u_cd);
		min_dt = arrTemp[0];
		max_dt = arrTemp[1];
		...
		// 해당 지수 리턴프로파일 계산
		Double currentPrc = GetIdxClsPrc(u_cd, t1);
		Double currentPrcBm = GetIdxClsPrc(u_cd_bm, t1);
		List<Double[]> clsPrcListArray = GetIdxClsPrcWithBmMultiPoint(u_cd, u_cd_bm, dtList);
		...
		returnList[7] =  currentPrc / GetIdxClsPrc(u_cd, min_dt) - 1;
	...
	public JSONObject GetInfoJson(String u_cd, String u_cd_bm, String t0, String t1){
	...
	public String[] GetMinMaxDt(String u_cd){
		...
		dbOutput = getSqlSession().selectList("StockProfileQueryMapper.selectMinMaxDtUcd", input);
	...
	public Double GetIdxClsPrc(String u_cd, String trd_dt){
		...
		dbOutput = getSqlSession().selectList("StockProfileQueryMapper.selectClsPrice", input);
		if(dbOutput.size() == 0){			
			try {
	 ...
	 */
	public ArrayList<Double[]> GetIdxClsPrcWithBmMultiPoint(String u_cd, String u_cd_bm, String[] timePoint){
		...
		dbOutput = getSqlSession().selectList("StockProfileQueryMapper.selectClsPriceWithBmMultiPoint", input);
	 ...
	 */
	public HashMap<String, Object> GetIdxMonthlyTimeSeriesWithBm(String u_cd, String u_cd_bm, String t0, String t1){
		...
		List<HashMap<String, Object>> dbOutput = getSqlSession().selectList("StockProfileQueryMapper.selectIdxMonthlyTimeSeriesWithBm", input);
		
		Integer size = dbOutput.size();
		String[] dateList = new String[size];
		Double[] clsPrcList = new Double[size];
		Double[] yieldList = new Double[size];
		Double[] clsPrcBmList = new Double[size];
		Double[] yieldBmList = new Double[size];
		...
		result.put("RF", riskFreeRateList);
